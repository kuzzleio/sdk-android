...
public class KuzzleDataCollectionTest {
    ...
    public void testSubscribe() throws JSONException, KuzzleException, IOException {
        ...
        collection.subscribe(new JSONObject(), null, null, null);
        verify(k, times(3)).query(eq("test"), eq("subscribe"), eq("on"), any(JSONObject.class), any(ResponseListener.class));
    }
    ...
    public void testUpdate() throws IOException, JSONException {
        ...
        KuzzleDataCollection collection = new KuzzleDataCollection(k, "test");
        collection.update("42", new JSONObject());
        verify(k, times(1)).query(eq("test"), eq("write"), eq("update"), any(JSONObject.class));
    }
